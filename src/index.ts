    ...
    ((original) => {
      Object.defineProperty(this.message_module, "default", {
        value: (msg: Message, params: MessageRenderingParameters) => {
          let opts = { message: msg, params } as MessageRenderingArguments;
          for (const hook of this.message_hooks) {
            const hooked = hook(opts);
          ...
          const component = original(opts.message, opts.params);
          return component;
        },
        writable: true,
      });
...
class CustomReactComponents {
  ...
  primarySettingElement() {
    ...
    const [enableHardwareAcceleration] = useState(() => this.HWAccel.default.getEnableHardwareAcceleration());
    const developerMode = this.configStore.DeveloperMode.useSetting();
    const homeAutoNav = this.configStore.DisableHomeAutoNav.useSetting();
    const {
    ...
    return jsxs(this.DiscordUI.FormSection, {
      ...
      children: [
        ...
        jsx(this.DiscordUI.FormSwitch, {
          value: testModeApplicationId != null,
          onChange: useCallback((e: boolean) => {
            if (!e) {
              this.testMode.reset();
            }
...
class DiscordPatcher {
  ...
  findModule(filter: (module: any) => boolean, options: { cacheOnly?: boolean; usingRawModule?: boolean; } = {}): any | null {
    ...
    for (const raw_module of Object.values(this.req.c)) {
      const m = checkModule(raw_module);
      if (m) return m;
    }
  ...
  // Bulk Patch
  async doPatch() {
    const modules = await getDiscordModules(this);
